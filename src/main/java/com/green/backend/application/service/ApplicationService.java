...
public class ApplicationService {
    ...
    public List<ApplicationDTO> ReadVisitRequest(String token) { // 1. 토큰에서 회원 번호 추출 (위와 동일)
        ...
        if (memberId == null) { throw new IllegalArgumentException("유효하지 않은 토큰입니다."); }
        Member member = memberRepository.findById(memberId) // 회원 정보 조회
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 회원입니다."));
        List<Application> applications = applicationRepository.findAllByMemberId(member); // 회원 기준 신청목록 조회
    ...
    public boolean assignExpert(ApplicationDTO dto) {
        ...
        application.setDueStartDate(dto.getDueStartDate()); // 답사 시작일 설정
